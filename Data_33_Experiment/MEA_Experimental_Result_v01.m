...
cycle_time = 4*3600+30*60; % total duration of a single experimental cycle, seconds
oxidation_duration = 60*60; % total duration of oxidative phase, seconds
...
reduction_start_time = 3.5*3600; % time at which reduction starts, seconds

flow_rate_flush_N2 = 5; % sccm
flow_rate_propylene = 10; % sccm
flow_rate_propane = 10; % sccm

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Conversion Factor mV-s vs. PPM (by volume) from Calibration measurements
Propylene_Calibration_Conversion_slope = 14.23965166457/(1e5); % mV-s per ppm
Propylene_Calibration_Conversion_b = 0.0002331391825/(1e5);
Propane_Calibration_Conversion_slope = 11.56668912127/(1e5); % mV-s per ppm
Propane_Calibration_Conversion_b = 0.0006868733321/(1e5);
...
[reference_all_2,fit_natural_2,fit_combined_ECM_2,fit_deconvoluted_ECM_2,released_propane_area] = ...
    Propylene_Separation_Analyzer(time,propane,number_cycle,cycle_time,reduction_duration,reduction_start_time);
...
molar_propylene = released_propylene_mole'/Area_MEA
...
y2_propane_propylene = 400; % range of first graph subplot
y1_water = 0; % range of second graph subplot
y2_water = 2.5e4; % range of second graph subplot
...
legend(strcat("+",num2str(mean(Oxidation_Summary(1,:))),"V Oxidative Current"),strcat(num2str(mean(Reduction_Summary(1,:))),"V Reductive Current"),"0V Applied",'location','northeastoutside');
xlim([x1 x2]);
ylim([CA_y_1 CA_y_2]);
...
y_combined_2 = 400; % range limit of first graph subplot
y_decon_1 = 0; % range limit of second graph subplot
y_decon_2 = 200; % range limit of second graph subplot
...
export_file_address = "C:\Users\Halcy\Dropbox\Nickel_Dithiolene_Research_Experimental_Data\Data_Trends_Analysis\Deconvoluted_ECM_20.xls";
export_array = [fit_deconvoluted_ECM(:,1) fit_deconvoluted_ECM(:,3)];
writematrix(export_array,export_file_address);

export_file_address = "C:\Users\Halcy\Dropbox\Nickel_Dithiolene_Research_Experimental_Data\Data_Trends_Analysis\Deconvoluted_ECM_35.xls";
export_array = [fit_deconvoluted_ECM_2(:,1) fit_deconvoluted_ECM_2(:,3)];
writematrix(export_array,export_file_address);
%}
end
...
natural_desorption_curve_fitting_range_before = 30*60; % range of natural desorption time before reduction starts, seconds
natural_desorption_curve_fitting_range_after = 30*60; % range of natural desorption time after reduction ends, seconds
...
while cycle_count <= number_cycle
    ...
    if ECM_induced_desorption_reference_count > 4
        ...
        add_points_propylene_x_1 = linspace(ECM_induced_desorption_reference_points(max_diff_index,1),ECM_induced_desorption_reference_points(max_diff_index+1,1),(length(ECM_induced_desorption_reference_points)+50));
        add_points_propylene_y_1 = linspace(ECM_induced_desorption_reference_points(max_diff_index,2),ECM_induced_desorption_reference_points(max_diff_index+1,2),(length(ECM_induced_desorption_reference_points)+50));
